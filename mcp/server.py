    ...
    for u in updates:
        ...
        subject: dict = {
            "object": {
                ...
                "objectId":   to_storage_id(subject_id),
            }
        }
        ...
        payload.append({
            ...
            "relationship": {
                "resource": {
                    ...
                    "objectId":   to_storage_id(u.resource_id),
                },
                "relation": u.relation,
                "subject":  subject,
            },
...
async def check_permission(args: PermissionCheckArgs) -> str:
    ...
    async with httpx.AsyncClient() as client:
        r = await client.post(
            ...
            headers=_spicedb_headers(),
            json={
                "resource":   {"objectType": args.resource_type, "objectId": to_storage_id(args.resource_id)},
                "permission": args.permission,
                "subject":    {"object": {"objectType": args.subject_type, "objectId": to_storage_id(subject_id)}},
            },
            timeout=15,
        )
        r.raise_for_status()
...
async def read_relationships(
    ...
    if resource_id:
        filter_obj["optionalResourceId"] = to_storage_id(resource_id)
    if relation:
        filter_obj["optionalRelation"] = relation
    if subject_type:
        filter_obj["optionalSubjectFilter"] = {"subjectType": subject_type}
        if subject_id:
            filter_obj["optionalSubjectFilter"]["optionalSubjectId"] = to_storage_id(subject_id)
    ...
    async with httpx.AsyncClient() as client:
        ...
        for line in r.text.strip().splitlines():
            ...
            results.append(
                f"{res.get('objectType')}:{to_external_id(res.get('objectId', ''))}"
                f"#{rel.get('relation')}"
                f"@{sub.get('objectType')}:{to_external_id(sub.get('objectId', ''))}"
            )
    return results
